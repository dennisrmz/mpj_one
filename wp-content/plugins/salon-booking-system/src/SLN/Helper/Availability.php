...
class SLN_Helper_Availability
{
    ...
    public function isValidOnlyTime($date)
    {
        ...
        return ($date >= $this->initialDate) && !($countHour && $this->getBookingsHourCount(
                $date->format('H'),
                $date->format('i')
            ) >= $countHour) && !$this->isDisabledTime($date->format('H'), $date->format('i'));
    }
    ...
    public function isDisabledTime($hour = null, $minutes = null)
    {
        return $this->getDayBookings()->isDisabledTime($hour, $minutes);
    }
}
