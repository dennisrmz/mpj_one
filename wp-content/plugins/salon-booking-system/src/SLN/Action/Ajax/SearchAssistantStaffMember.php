...
class SLN_Action_Ajax_SearchAssistantStaffMember extends SLN_Action_Ajax_Abstract
{
    ...
    public function userSearch($wp_user_query) {
		...
		$wpdb->prepare(
		    "SELECT DISTINCT ID FROM $wpdb->users u INNER JOIN $wpdb->usermeta um ON u.ID = um.user_id WHERE LOWER(u.user_email) LIKE %s AND meta_key='{$wpdb->prefix}capabilities' AND meta_value LIKE %s",
		    '%' . $wp_user_query . '%',
